...
if $DO_BUILD || [ ! -f "$MAIN_JS" ]; then
    ...
    # The default `java` on PATH is a Salesforce shim that breaks sbt's rt-ext
    # export, silently producing NO output — which would upload a STALE main.js.
    # Pin the real Zulu JDK and FAIL LOUD if the build/link does not succeed.
    ...
    _mnu_before=$(stat -f '%m' "$MAIN_JS" 2>/dev/null || echo 0)
    ( cd "$MNU_ROOT/solo" && sbt fullLinkJS ); _rc=$?
    if [ $_rc -ne 0 ]; then echo "ERROR: sbt fullLinkJS failed (rc=$_rc) — aborting deploy."; exit 1; fi
    _mnu_after=$(stat -f '%m' "$MAIN_JS" 2>/dev/null || echo 0)
    if [ "$_mnu_before" = "$_mnu_after" ]; then
        echo "ERROR: main.js was NOT regenerated by the build (stale). Aborting."; exit 1
    fi
fi
if [ ! -f "$MAIN_JS" ]; then
