...
object SimRunner {
    def main(args : Array[String]) {
        ...
        if (args.length >= 2 && args(0) == "--replay") {
            ...
            if (headerLine.contains("UseServitor")) opts :+= UseServitor
            if (headerLine.contains("UseAtlachNacha")) opts :+= UseAtlachNacha
            if (headerLine.contains("UseBokrug")) opts :+= UseBokrug
            if (headerLine.contains("UseGlaakiIGOO")) opts :+= UseGlaakiIGOO
        ...
        val allIGOOOptions : $[GameOption] =
            $(IGOOs, UseByatis, UseAbhoth, UseDaoloth, UseNyogtha, UseTulzscha, UseYgolonac,
        ...
        resultList ++= games.map { case (pc, ff) =>
            ...
            try {
                ...
                while (!c.isInstanceOf[GameOver]) {
                    ...
                    if (n > 15000)
                        throw new RuntimeException("Game exceeded 15000 steps")
                }
