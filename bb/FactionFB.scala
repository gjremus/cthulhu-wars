...
object FBExpansion extends Expansion {
    ...
    def perform(action : Action, soft : VoidGuard)(implicit game : Game) : Continue = action @@ {
        ...
        case PreMainAction(f) if f != FB && game.factions.has(FB) && FB.can(CyclopeanGaze) =>
            ...
            val gazeRegions = (areas ++ game.factions.has(BB).??($(BB.moon))).%(r => FB.at(r, RevenantOfKnaa).any || FB.at(r, Ghatanothoa).any)
        ...
        case FBCyclopeanGazePainUnitAction(self, actor, uRef, r, sourceUnit, sourcesPending, fromBattle) =>
            ...
            // Moon retreat expansion: Moon is adjacent to all regions, so pained
            // units on the Moon can retreat to any map area without FB units.
            val moonRetreatDests = (u.region.is[MoonHold]).??(areas.%(d => FB.at(d).%(_.uclass.utype != Building).none))
            val destinations = (game.board.connectedForRetreat(u.region).%(_.glyph.onMap).%(d => FB.at(d).%(_.uclass.utype != Building).none) ++ moonDest ++ moonRetreatDests).distinct
            if (destinations.any)
