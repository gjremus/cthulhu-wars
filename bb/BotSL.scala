...
class GameEvaluationSL(implicit game : Game) extends GameEvaluation(SL)(game) {
    def eval(a : Action) : $[Evaluation] = {
        ...
        // SL exception: Lethargy lets SL "do nothing" in a region — no movement
        // required, so crater/CG avoidance doesn't apply to Lethargy dispatch.
        a.unwrap match {
            case MoveAction(_, u, o, r, _) =>
                if (have(Burrow) && self.allInPlay.tag(Moved).num >= 1) {
                    fbMultiMoveAvoidance(r, 2).foreach(e => true |=> e)
                } else {
        ...
        a match {
            ...
            // Energy Nexus Pre-Battle variant — always use it when available
            case EnergyNexusPreBattleAction(_) =>
                true |=> 2000 -> "energy nexus pre-battle take action"

            // Easier SBR variants — scoring mirrors the original requirements
            // PayLastPowerAction (spend 1) matches Pay3SomeoneGains3 (spend 3, one gets 3)
            case PayLastPowerAction(_) =>
                self.numSB == 5 && realDoom >= 30 |=> 5000 -> "last spellbook and end"
                self.numSB == 5 |=> 3000 -> "last spellbooks"
                self.numSB == 4 |=> 400 -> "pre-last spellbook"
                realDoom >= 20 |=> 600 -> "the end is near"
                active.none && realDoom + self.gates.num * 2 + self.goos.num >= 30 |=> 1000000 -> "the end is now"
                true |=> 4 -> "if wiped out"

            // PayLast2PowerAction (spend 2) matches Pay3EverybodyGains1 (spend 3, everyone gains 1)
            case PayLast2PowerAction(_) =>
                self.numSB == 5 && realDoom >= 30 |=> 5000 -> "last spellbook and end"
                self.numSB == 5 |=> 3000 -> "last spellbooks"
                self.numSB == 4 |=> 400 -> "pre-last spellbook"
                realDoom >= 20 |=> 600 -> "the end is near"
            ...
            // PayLast3PowerAction (spend 3) matches Pay3EverybodyLoses1 (spend 3, everyone loses 1)
            case PayLast3PowerAction(_) =>
                self.numSB == 5 && realDoom >= 30 |=> 5000 -> "last spellbook and end"
                self.numSB == 5 |=> 3000 -> "last spellbooks"
                self.numSB == 4 |=> 400 -> "pre-last spellbook"
                realDoom >= 20 |=> 600 -> "the end is near"
