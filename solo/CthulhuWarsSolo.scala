...
object CthulhuWarsSolo {
    ...
    def setupUI() {
        ...
        // <script id=script> ?v= tag the deploy stamps into index.html), then every
        // 10s re-fetch THIS PAGE'S OWN URL (cache-busted) and read the live ?v= from
        // its script tag. Re-fetching the current URL is used rather than a fixed
        // cache-bust.txt path because the served path differs per build (HB/BB/TT/MNU
        // sit under /XX/, Library sits at root) — the page's own URL is always
        // reachable. If the live version differs from the loaded one, hard-reload.
        {
        ...
        def startGame(setup : Setup, recorded : $[String] = $, self : |[Faction] = None) {
            ...
            def drawMap(implicit game : Game) {
                ...
                if (board.isLibraryMap && place.nonEmpty) {
                    ...
                    // 2026-08-11 (Task #133): ONE continuous coverage-based uniform shrink,
                    ...
                    // (small-area ≤ Gloomloft-5U, large-unit maxDim > 96, and the
                    // placement-loop overlap-redo): the single coverage sum subsumes all of
                    // them — a big unit inflates the numerator, many units inflate the
                    // numerator, and a small room shrinks the denominator. The factor is
                    // applied UNIFORMLY to every unit in the room (all classes shrink by the
                    // same percentage), so the piece SIZE RANGE is preserved just like the
                    // Moon. Floored so units never vanish. The budget is the Library map's own
                    // tuned value (0.42, tighter than the Moon's 0.60 because a room's
                    // bounding box overstates its true painted area).
                ...
                areas.foreach { r =>
                    ...
                    def placeUnits(units : $[DrawItem]) : Unit = {
                        units.sortBy(d => -rank(d)).foreach { d =>
                            ...
                            // (No shrink-redo — the up-front coverage shrink already sized
                            // the room, so residual overlap is minimal.)
            ...
            def factionStatus(f : Faction, b : CachedBitmap)(implicit game : Game) {
                ...
                val lcis = f.loyaltyCards.reverse.indexed./ { (lc, i) =>
                    ...
                    val extraParams = if (lc == AzathothIGOOCard) ", " + game.azathothGlyphPosition
                            ...
                            // Packed sprite list for the overlay: "assetId|display|tint:screen:overlay",
                            // ;-separated. assetId + tint come from the unit's own map DrawItem so the
                            // overlay replays the exact map tint with no per-build faction table.
...
case (DimensionalShamblerUnit, Filth) => DrawItem(null, f, Filth, Alive, $, 53 + last.x, last.y - 15)
            ...
            def updateStatus() {
                ...
                if (setup.seating.has(BB)) {
                    ...
                    def moonSpriteAssetId(u : UnitFigure) : String = {
                        ...
                        u.uclass match {
                            ...
                            // Faceless Blight (FBE): Acolyte reuses the DC acolyte placeholder
                            // sprite (no bespoke FBE cultist art). HighPriest stays on gc placeholder.
                            ...
                            // Faceless Blight (FBE): Fungal Thrall reuses the Dimensional Shambler
                            // placeholder; Byagoona uses its real sculpt art (fbe-byagoona), matching
                            // the Earth map.
                ...
                if (setup.seating.has(TB)) {
                    ...
                    dom.document.getElementById("tb-mantle-hud-btn").?.foreach { el =>
                        val mantleAdjStr = safe(displayGame.tbMantleAreas./(_.name).mkString(", "))
                        val s = s"""onExternalClick("TB","Mantle",$mantleCount,"$mantleList","$mantleAdjStr")"""
                        el.setAttribute("onclick", "event.stopPropagation(); " + s)
                        el.setAttribute("onpointerover", "event.stopPropagation(); " + s.replace("Click","Over"))
                        el.setAttribute("onpointerout",  "event.stopPropagation(); " + s.replace("Click","Out"))
                    }
                        ...
                        head match {
                            ...
                            case UIQuestion(f, game, actions, waiting) => {
                                ...
                                val extra = actions.unwrap.use(l => game.extraActions(f, false, actions.has(SacrificeHighPriestAllowedAction)).%{
                                    ...
                                    case _ : SacrificeHighPriestOutOfTurnMainAction if l.of[SacrificeHighPriestDoomAction].any => false
                                    case _ => true
                                })
        ...
        // The Invasion (TI): appended to the alt picker per Implementation Guide §2.9/§3
        val altPickerFactions = $(GC, CC, BG, YS, OW, SL, WW, TT, AN, DS, BB, TS, FB, DC, FBE, XSS, TB, CS, TI)
