        ...
        def startGame(setup : Setup, recorded : $[String] = $, self : |[Faction] = None) {
            ...
            def libraryVerticalAnchor(cv : dom.html.Canvas, bw : Int, bh : Int, imgScale : Double) : (Double, Double) = {
                ...
                if (roaNum != null && roaNum.parentNode != null) {
                    ...
                    // Vertical: start just below the bottom edge of the IMAGE with substantial gap
                    val bottom = (cr.bottom - bRect.top) * vpy
                    // 80px gap below the ritual circle bottom per owner @@@ feedback (30px/60px were too high)
                    (cx, bottom + 80.0 * vpy)
                } else {
                ...
                ask("Cthulhu Wars", $("Quick Game".hl, "Local Game".hl, redirect.?("<a href='" + origin + "' target='_blank'><div>" + "Online game".hl + "</div></a>").|("Online Game".hl), "More Factions/ Units".hl, "Extra", "About", "Test").take(menu), {
                    ...
                    case 3 =>
                        ask("More Factions/ Units", $(
                            "<a href='/mnu/' target='_blank'><div><span style='color:#b8b8b8'>More Neutral Units</span></div></a>",
