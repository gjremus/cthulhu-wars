...
// Alternate spellbooks (DSAlternateSpellbooks option)
// SPELLBOOK — use .can(), CAN be blocked by Moonbeast
case object Omnipotence extends FactionSpellbook(DS, "Omnipotence")
// SPELLBOOK — use .can(), CAN be blocked by Moonbeast
case object FiendishSpawn extends FactionSpellbook(DS, "Fiendish Spawn") with BattleSpellbook
// SPELLBOOK — use .can(), CAN be blocked by Moonbeast
case object DirectedEnergy extends FactionSpellbook(DS, "Directed Energy") with BattleSpellbook
...
case class OmnipotenceDoneSelectAction(self : Faction, selected : $[UnitRef]) extends BaseFactionAction("Choose Avatars to move", "Done")
case class OmnipotenceMoveAllAction(self : Faction, selected : $[UnitRef], r : Region) extends BaseFactionAction("Move all to", r)
...
case class FiendishSpawnDoneAction(self : Faction) extends BaseFactionAction("Fiendish Spawn", "Done")

case class DirectedEnergyPostBattleAction(self : Faction, n : Int) extends BaseFactionAction("Directed Energy", implicit g => "Gain " + n.power + " with Directed Energy")
case class DirectedEnergySkipAction(self : Faction) extends BaseFactionAction("Directed Energy", "Skip")
...
object DSExpansion extends Expansion {
    ...
    def perform(action : Action, soft : VoidGuard)(implicit game : Game) = action @@ {
        ...
        case OmnipotenceSelectAction(self, selected, remaining) =>
            var ask = Ask(self)
            remaining.foreach { ref =>
                ask = ask.add(OmnipotenceSelectAction(self, selected :+ ref, remaining.but(ref)))
            }
            if (selected.any)
                ask = ask.add(OmnipotenceDoneSelectAction(self, selected))
            ask = ask.add(OmnipotenceCancelAction(self))
            ask

        case OmnipotenceDoneSelectAction(self, selected) =>
            if (selected.num == 1) {
        ...
        case OmnipotenceMoveAllAction(self, selected, r) =>
        ...
        case OmnipotenceMoveOneAction(self, uRef, r, remaining) =>
        ...
        case OmnipotenceCancelAction(self) =>
            self.power += 1
            self.oncePerTurn = self.oncePerTurn.but(Omnipotence)
