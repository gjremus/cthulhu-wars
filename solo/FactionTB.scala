...
object TBExpansion extends Expansion {
    ...
    def perform(action : Action, soft : VoidGuard)(implicit game : Game) = action @@ {
        ...
        case TBAutotomyAction(self, segment, retreatDest, arena) =>
            ...
            // Elder Signs are NOT granted here. The pool count must include any Segment
            // killed by the remaining kill assignment that runs after this transfer, so
            // the "1 Elder Sign per Segment in pool" grant is deferred to TBAutotomyPhase
            // (after all Kills are assigned this battle).
