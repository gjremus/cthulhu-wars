...
case object OW extends Faction { f =>
    ...
    def mutantCost(r : Region)(implicit game : Game) = {
        // Cheap mutants: always cost 1 power regardless of gate ownership
        1
    }
...
object OWExpansion extends Expansion {
    ...
    def perform(action : Action, soft : VoidGuard)(implicit game : Game) = action @@ {
        ...
        case MainAction(f : OW) =>
            ...
            if (f.can(DreadCurse)) {
                var n = f.all(Abomination).num + f.all(SpawnOW).num
                // Opener Buff: Yog Curse Die — Yog-Sothoth adds 1 die when in play
