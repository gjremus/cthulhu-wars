...
class GameEvaluationDS(implicit game : Game) extends GameEvaluation(DS)(game) {
    def eval(a : Action) : $[Evaluation] = {
        ...
        // FB-awareness: score CG/Eye Opens prompts asked of DS
        result ++= fbPromptedEvals(a)

        // FB/TS-awareness: avoid craters, CG regions, and Ice Age
        a.unwrap match {
        ...
        a match {
            ...
            // ---- ALTERNATE SPELLBOOKS: Omnipotence, Fiendish Spawn, Directed Energy ----

            // Omnipotence — mirrors Traitors scoring (both are 1-cost tactical repositioning)
            case OmnipotenceMainAction(_) =>
                val avatars = self.allInPlay.%(u => u.uclass == AvatarThesis || u.uclass == AvatarAntithesis || u.uclass == AvatarSynthesis)
                avatars.any |=> 600 -> "omnipotence reposition avatars"
                avatars.num >= 2 |=> 1200 -> "omnipotence move multiple avatars"
                avatars.%(u => enemyGooThreat(u.region)).any |=> 2000 -> "omnipotence escape goo threat"

            case OmnipotenceSelectAction(_, selected, _) =>
                true |=> 500 -> "select avatar to move"

            case OmnipotenceDoneSelectAction(_, _) =>
                true |=> 400 -> "done selecting"

            case OmnipotenceMoveAllAction(_, _, _) =>
                true |=> 300 -> "move all together"

            case OmnipotenceMoveSeparatelyAction(_, _) =>
                true |=> 200 -> "move separately"

            case OmnipotenceMoveOneAction(_, _, _, _) =>
                true |=> 300 -> "move avatar to region"

            case OmnipotenceJoinAction(_, _, _) =>
                true |=> 400 -> "join friendly units"

            case OmnipotenceCancelAction(_) =>
            ...
            case DirectedEnergySkipAction(_) =>
                true |=> -500 -> "prefer using directed energy"
            ...
            case LoyaltyCardDoomAction(_) =>
                ...
                self.doom < 5 |=> 4000 -> "NU: loyalty card early"
            ...
            case NeutralMonstersAction(_, lc) =>
                ...
                // §18 boost: DS top three jittered for diversity
            ...
            case LoyaltyCardSummonAction(_, uc, r) =>
                ...
                true |=> 500 -> "NU: place NM"

            case FreeSummonAction(_, _, r, _) =>
                r.ownGate |=> 1200 -> "NU: free summon at gate"
                true |=> 800 -> "NU: free summon"

            case ShantakCarryCultistAction(_, o, ur, r) =>
                r.ownGate && r.allies.cultists.none |=> 2000 -> "NU: shantak to empty gate"
                true |=> 800 -> "NU: shantak carry"
            ...
            case ShamblerDeployAction(_, r, _) =>
                ...
                r.ownGate |=> 1500 -> "NU: shambler defend"
            ...
            case IndependentGOOMainAction(_, lc, _) =>
                ...
                self.gates.num < 2 |=> -3000 -> "NU: need 2+ gates"
                remaining >= 3 |=> 700 -> "NU: igoo comfortable"
                remaining >= 1 |=> 500 -> "NU: igoo affordable"
                (lc == TulzschaCard) |=> 800 -> "NU: tulzscha"
                (lc == AbhothCard) |=> 700 -> "NU: abhoth"
            ...
            case IndependentGOOAction(_, lc, r, _) =>
                r.ownGate && r.allies.cultists.any |=> 700 -> "NU: igoo own gate"
                r.ownGate |=> 500 -> "NU: igoo gate"
                (lc.unit == Byatis) && r.ownGate |=> 851 -> "NU: byatis immobile at gate"
                (lc.unit == Byatis) && !r.ownGate |=> -500 -> "NU: byatis avoid non-gate"
                true |=> 400 -> "NU: igoo place"

            case GodOfForgetfulnessMainAction(_, _, _) =>
                true |=> 1000 -> "NU: byatis pull"
            ...
            case NightmareWebMainAction(_, _) =>
                true |=> 800 -> "NU: nyogtha web"

            case NightmareWebAction(_, r) =>
                r.ownGate |=> 1500 -> "NU: web at gate"
                true |=> 500 -> "NU: web"
            ...
            case CeremonyOfAnnihilationChoiceAction(_) =>
                ...
                true |=> 600 -> "NU: ceremony"

            case SacrificeHighPriestMainAction(_) =>
                power <= 3 |=> 1500 -> "NU: HP sacrifice low power"
                true |=> 1000 -> "NU: HP sacrifice"

            case SacrificeHighPriestDoomAction(_) =>
                true |=> 1100 -> "NU: HP doom sacrifice"

            case SacrificeHighPriestAction(_, r, _) =>
                r.ownGate.not |=> 700 -> "NU: HP not at gate"
                true |=> 100 -> "NU: HP sacrifice any"

            case SacrificeHighPriestOutOfTurnMainAction(_) =>
                power <= 1 |=> 1200 -> "NU: HP oath low power"
                true |=> 800 -> "NU: HP oath"
