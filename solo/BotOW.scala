    ...
    def eval(a : Action) : $[Evaluation] = {
        ...
        a match {
            ...
            case SummonAction(_, uc, r) if r.enemyGate && r.allies.none =>
                ...
                // OpenerCheapMutants: SBR requires 3 gates instead of 2 — boost scoring
                (need(UnitsAtEnemyGates) && game.options.has(OpenerCheapMutants)) |=> 500 -> "harder SBR needs more gate presence"
            ...
            case DreadCurseAction(_, n, r) =>
                ...
                // OpenerYogCurseDie: more aggressive DC — target GOO regions
                val totalUnitsInRegion = others./~(_.at(r)).num
                val hasGOO = others.exists(_.at(r).goos.any)
                val expectedHits = (n + 1) / 2  // DC dice / 2 rounded up
                val canReachGOO = hasGOO && expectedHits >= totalUnitsInRegion
                (game.options.has(OpenerYogCurseDie) && canReachGOO) |=> 120000 -> "YogCurseDie: DC can reach GOO"
                (game.options.has(OpenerYogCurseDie) && hasGOO && expectedHits >= totalUnitsInRegion - 1) |=> 80000 -> "YogCurseDie: DC likely reaches GOO"
