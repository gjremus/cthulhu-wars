...
object ElderThingMindControl {
    def suppresses(u : UnitFigure)(implicit game : Game) : Boolean =
        u.uclass.utype == GOO && game.factions.exists(f2 => f2 != u.faction && f2.at(u.region, ElderThing).any)
}
...
class Player(private val f : Faction)(implicit game : Game) {
    ...
    def summonRegions : $[Region] = (f.allGates.onMap ++ f.can(TheyBreakThrough).??(f.enemies./~(_.allGates) ++ game.abandonedGates)).nex.distinct
...
class Game(val board : Board, val ritualTrack : $[Int], val setup : $[Faction], val logging : Boolean, val providedOptions : $[GameOption]) extends Expansion {
    ...
    // Non-standard Ritual of Annihilation paths (Hecatomb pure-DH) bypass RitualAction
    // dispatch and ritualHistory polling, so they don't naturally notify cross-faction
    // RoA spellbook reactors. This helper explicitly satisfies them.
    ...
    def moves(f : Faction)(implicit w : AskWrapper) {
        ...
        if ((f.units.nex.onMap.not(Moved).%(_.canMove).any || parasitized.any) && f.power > 0)
            + MoveMainAction(f)
    }
    ...
    def igooCost(f : Faction, igoo : IGOOLoyaltyCard) : Int = igoo match {
        ...
        case CthughaCard =>
            val factionGOOs = f.units./(_.uclass).%(_.utype == GOO).%(_.isInstanceOf[FactionUnitClass]).distinct
    ...
    def independents(f : Faction)(implicit w : AskWrapper) {
        ...
        if (f.has(GhatanotoaIGOO) && f.power >= 1 && f.allInPlay.%(_.uclass == GhatanotoaIGOO).exists(u => ElderThingMindControl.suppresses(u)))
            ...
            ghatRegion.foreach { r =>
                if (f.enemies.exists(_.at(r).%(_.uclass.utype == Cultist).%(u => !game.mummifiedCultists.has(u.ref)).any))
    ...
    def perform(action : Action, soft : VoidGuard)(implicit game : Game) : Continue = action @@ {
        ...
        case EndPhasePromptsAction(next, Nil) =>
            ...
            if (factions.exists(_.active)) {
                println(s"[END-PHASE-TRACE] Resuming action phase with ${factions.%(_.active).head.short}")
        ...
        case MoveContinueAction(self, moved) =>
            if (self.power == 0)
                ...
                val ownUnits = self.units.nex.onMap.not(Moved).%(_.canMove).sortA
