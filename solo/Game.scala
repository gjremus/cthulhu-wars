...
// HB Fix (2026-08-11): "Only Manual Gate Control" — when set, a unit moving into
// a region with an empty gate (or a unit left standing when a gate is abandoned)
// does NOT automatically occupy the gate. The gate stays uncontrolled until the
// player explicitly uses the "Control gates" menu. Suppresses the auto-assign in
// checkGatesGained; the manual AdjustGateControlAction path is unchanged.
...
class Game(val board : Board, val ritualTrack : $[Int], val setup : $[Faction], val logging : Boolean, val providedOptions : $[GameOption]) extends Expansion {
    ...
    //    per Unit of ANY faction that died this Action, holding WHERE it died and
    //    the faction that CONTROLLED it (None = controller not tracked). Creator-
    //    corrected 2026-09-23: triggers at 2+ IN THE SAME AREA (deaths in different
    //    Areas within one Action do not combine); the +1 Doom clause needs 3+
    //    same-Area entries controlled by the faction collecting the ability (FBE,
    //    or an enemy that copied it via Ancient Sorcery).
    ...
    var fbeCardDice : $[Int] = $
    var fbeSelfConsumingDeaths : $[(Region, |[Faction])] = $
                    ...
                    if (DS.chaosGateRegions.has(r).not || self == DS) {
                        if (factions.%(_.gates.has(r)).none) {
                            ...
                            if (blockers.any) {
                                ...
                                // automatic gate capture. A unit entering a region with an
                                // empty/abandoned gate does NOT occupy it — the gate stays
                                // uncontrolled until the player uses the "Control gates"
                                // menu. Suppress the auto-assign entirely.
                            }
                            else {
    ...
    def independents(f : Faction)(implicit w : AskWrapper) {
        ...
        val cthughaAvailable = loyaltyCards.has(CthughaCard) && {
            ...
            allGOOs.%(goo => {
                val gooCost = if (goo.uclass.isInstanceOf[FactionUnitClass]) f.awakenCost(goo.uclass, goo.region).|(f.gooValue(goo.uclass)) else goo.uclass.cost
    ...
    def doomDone(f : Faction, blockDone : Boolean = false)(implicit w : AskWrapper) {
        if (f.has(GhatanotoaIGOO) && f.upgrades.has(ExecrationOfMu).not) {
            val gatesOnMap = f.allGates.onMapOrMoon.num
            val cultistsOnMap = f.units.%(u => u.region.onMapOrMoon && u.uclass.utype == Cultist).num
    ...
    def perform(action : Action, soft : VoidGuard)(implicit game : Game) : Continue = action @@ {
        ...
        case MoveDoneAction(self) =>
            ...
            val tbStalkDests : $[Region] = if (setup.has(TB) && TB.can(Stalk) && TB.allInPlay.%(_.uclass.utype == Cultist).any)
        ...
        case MoveAction(self, u, o, r, cost) =>
            ...
            // credit follows the movement in the game log. (Special-movement powers that
            // bypass MoveAction credit BB via game.creditCatnappingOffMoon instead.)
