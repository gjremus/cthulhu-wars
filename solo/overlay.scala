...
object Overlays {
    ...
    def info(s : $[Any]) : |[String] = (s @@ {
        ...
        // NOTE: The "Angles of Time" text below is INTENTIONALLY MODIFIED (not verbatim Necronomicon)
        // per owner request, and is EXCLUDED from Necronomicon audits. The Cronophage text is verbatim.
        case $("Hound of Tindalos") => loyaltyCard("Hound of Tindalos", 1, 4, 4, "Pay 2 Doom + 2 Power (Terror). Place at ANY Gate on the map.", "Cronophage", "Ongoing", "The Hound cannot perform the Move Action by itself. The Hound Moves for free whenever you Move any other Unit. In doing so, the Hound teleports directly from an Area with a Gate to another Area with a Gate – neither of which need to be Controlled by you (or anyone).<br><br><b>Angles of Time</b> (Ongoing): The Hound cannot be assigned a Kill in Battle. However, if a Hound ever exists in an Area without a Gate it is Eliminated. It can also be chosen to be Eliminated if it cannot be Pained due to Enemy presence in adjacent Areas (per normal Pain rules).")
