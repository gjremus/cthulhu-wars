...
object MoonPlacement {
    ...
    def scatterSized(useHorizontal : Boolean, seed : Int, heightsPx : Array[Double], aspects : Array[Double]) : Array[(Double, Double)] = {
        ...
        // The rules text occupies the lower part of the disc. Block the ENTIRE text
        // band for ALL units (no size exception): the band runs from where the text
        // starts (y=500) down past the bottom of the visible disc (centre 440 + radius
        // 379 = 819), full width. No unit — large or small — may be placed where its
        ...
        // vertical span overlaps the text band (full-sprite check, not just centre).
...
object Overlays {
    ...
    def infoDunwich(s : $[Any]) : |[String] = (s @@ {
        ...
        // Shared common text — back of all four Whateley cards. Each Whateley card
        // links here via onExternalClick('How to Recruit Whateley Clan').
        case $("How to Recruit Whateley Clan") => whateleyClanCommonText
    ...
    case $("High Priest") => loyaltyCard(HighPriestCard.name, HighPriestCard.quantity, HighPriestCard.cost, HighPriestCard.combat, "The High Priest is a new type of Cultist, it is Recruited like an Acolyte. Each High Priest generates 1 Power during the Gather Power Phase, can Create and Control a Gate, and can be Captured.", "Unspeakable Oath", "Ongoing", "At the end of any player's Action (even if it is not your turn), Sacrifice your High Priest (return him to your Pool) and gain 2 Power. This may also be done during the Gather Power and Doom Phases.")
        ...
        // Defilers Court (DC): setup overlay
        case $("DC", "Setup") => requirement("Start with 4 Power, 0 Sin, and no Units on the map. Your Start Area is the first Area you Awaken Y'Golonac. Put your 6 Acolytes on each of your Spellbook requirements (they are not initially in your Unit Pool). When you gain a Spellbook, place that Acolyte into any Area. Once in play, your Acolytes function (and die) as normal. Any Serpent Men placed on your Faction Sheet (via Ancient Sorcery) remain for the entire game, and Sleeper will need to place 2 to copy each of your abilities.")
    ...
    def loyaltyCardWhateley(name : String, slug : String, cost : Int, combat : Int, ability1 : String, phase1 : String, text1 : String, ability2 : String, phase2 : String, text2 : String, markerHtml : String = "") = s"""
        <table class="loyalty-card-table">
            <thead>
                <tr>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        ...
                        <div>&nbsp;</div>
                        <div class="black-border">
                            <span class="cost-color">(Doom Phase)</span><span class="nt">: see </span><span class="ability-color pointer" onclick="onExternalClick('How to Recruit Whateley Clan')">How to Recruit Whateley Clan</span><span class="nt">.</span>
                        </div>
                    </td>
                    <td></td>
                </tr>
    ...
    // Wilbur Whateley pop-up marker — a shrunk Gate image with the stored-Gate count
    // (Yr and Nhnngr) overlaid, banked on his Loyalty Card. Reads the live count from
    // Overlays.currentGame; hidden when zero or no game is active.
    def wilburGateMarker : String = Overlays.currentGame match {
        case Some(g) if g.wilburStoredGates > 0 =>
            val n = g.wilburStoredGates
            s"""<div>&nbsp;</div>
                <div class="black-border" style="text-align:center;">
                    <span style="position:relative;display:inline-block;">
                        <img src="${imageSource("gate")}" style="height:10vh;height:10dvh;width:auto;" />
                        <span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font:bold 4vh 'Bohemian Typewriter',monospace;color:#fff;text-shadow:-1px -1px 0 #000,1px -1px 0 #000,-1px 1px 0 #000,1px 1px 0 #000;">${n}</span>
                    </span>
                    <div><span class="nt">Gates stored (Yr and Nhnngr)</span></div>
                </div>"""
        case _ => ""
    }

    // Junior Whateley pop-up marker — a small pink circle (matching his pink base) with the
    // growth-token count overlaid. Reads the live count from Overlays.currentGame.
    def juniorTokenMarker : String = Overlays.currentGame match {
        case Some(g) if g.juniorTokens > 0 =>
            val n = g.juniorTokens
            s"""<div>&nbsp;</div>
                <div class="black-border" style="text-align:center;">
                    <span style="display:inline-flex;align-items:center;justify-content:center;width:10vh;width:10dvh;height:10vh;height:10dvh;border-radius:50%;background:#CB307E;border:0.4vh solid #7a1a4a;">
                        <span style="font:bold 4vh 'Bohemian Typewriter',monospace;color:#fff;text-shadow:-1px -1px 0 #000,1px -1px 0 #000,-1px 1px 0 #000,1px 1px 0 #000;">${n}</span>
                    </span>
                    <div><span class="nt">Growth tokens</span></div>
                </div>"""
        case _ => ""
    }

    // Dunwich Horror — shared "How to Recruit Whateley Clan" common text (the back of all
    // four Whateley cards). Standalone overlay reached from each Whateley card's clickable
    // link; verbatim from necronomicon.app/neutral-expansions/the-dunwich-horror.
    def whateleyClanCommonText = s"""
        <table class="loyalty-card-table">
            <thead>
                <tr>
                    <th style=width:10%></th>
            ...
            <tbody>
                <tr>
                    <td></td>
                    <td>
                        <div class="h1 black-border" style="margin-right: -3ex; margin-left: -3ex; "><span class="h2 abaddon nt">How to Recruit Whateley Clan</span></div>
                        <div>&nbsp;</div>
                        <div class="black-border">
                            <span class="ability-color">Doom Phase</span><span class="nt">: In player order, each Faction can choose to recruit one Whateley in each Doom Phase. To do so, they must pay that Whateley's cost.</span>
                        </div>
                        <div>&nbsp;</div>
                        <div class="black-border">
                            <span class="nt">If the recruited Whateley is not yet in play, place it on the Map as the new owner wishes.</span>
                        </div>
                        <div>&nbsp;</div>
                        <div class="black-border">
                            <span class="nt">You can recruit a Whateley that is already in play! In this case, the former owner hands over the relevant Loyalty Card (the miniature remains in play in its current location). That Whateley is now yours until (possibly) the next Doom Phase.</span>
                        </div>
                    </td>
                    <td></td>
                </tr>
    ...
    def csFactionOverlay() = {
        ...
        faction(CS, "info:cs-background", ChromaticPerversion, "Ongoing",
            ...
            (CSTulzscha,              1, "5", calc(g => { implicit val gg : Game = g; 2 * CS.onMap(LuminousGlobule).not(Zeroed).num }), s"""
                ...
                <div class=p>${reference(CS, CorruptedRending)} ${cost("(1 Power, Action):")} Choose a region with a globule or meteorite and at least 2 factions. Initiate a battle between 2 factions. You choose attacking faction.</div>"""),
            (PrismaticWell,           0, "—", "—", "")
        ), setup = true)
    }
    ...
    def csOverlay(s : $[Any]) : String = s match {
        ...
        case $("CS", CorruptedRending.name)            => spellbook(CorruptedRending.name, "1 Power, Action (Faction Ability)", "Choose a region with a globule or meteorite and at least 2 factions. Initiate a battle between 2 factions. You choose attacking faction.")
        case $("CS", Insanity.name)                    => spellbook(Insanity.name, "Post Battle", "In areas with a meteorite or globule, extra, unapplied combat results are applied according to the following:<br>- Results against you are applied to the attacking faction<br>- Results in battles without you are applied to the rolling faction.")
        ...
        case $("CS", EffulgentSacrifice.name)          => spellbook(EffulgentSacrifice.name, "Action: Cost 0", "Any faction may take this action. In a region with a well controlled by a faction other than COoS, sacrifice your cultists in that region and eliminate the globule. Gain 1 doom. COoS gains an elder sign.")
    ...
    def fbeOverlay(s : $[Any]) : String = s match {
        ...
        case $("FBE", "Setup") => requirement("After Progenitus, 8 Power, 6 Acolytes and a Controlled Gate in an empty Area not adjacent to another faction's Start Area.")
    ...
    def faction(f : Faction, background : String, unique : Spellbook, uniquePhase : String, uniqueText : String, miscSpellbooks : $[Spellbook], units : $[(UnitClass, Int, String, String, String)], footer : String = "", setup : Boolean = false, sbLine : String = "") = {
        ...
        <table class="faction-table" style="background-image:url(${backgroundUrl})">
            ...
            <tbody>
                ...
                ${
                    // Setup + Spellbooks line, just below the unique-power box and
                    // above the unit table. "Setup" (when the faction has custom,
                    // non-predetermined setup) is a clickable link styled exactly
                    // like a spellbook reference; it opens the faction's setup text
                    // overlay via onExternalClick('<short>','Setup').
                    val setupLink =
