...
object Overlays {
    ...
    // Live game exposed to Overlays so faction-card GOO/shared-combat rows can
    // show a dynamic calculated value instead of "?" (set each render in
    // CthulhuWarsSolo). Mirrors the HB build.
    ...
    def info(s : $[Any]) : |[String] = (s @@ {
        ...
        case $("The Bloated Woman", spellbook : Boolean, facedown : Boolean, capturedCount : Int, capturedList : String, spriteList : String) => loyaltyCardIGOO("The Bloated Woman", "4", "1", spellbook, "1. Your Controlled Gate is in an Area with your Great Old One.<br>2. Pay 4 Power, and place the Bloated Woman in the Area containing the Gate.<br>Crawling Chaos (CC): Pay 3 Power at a Controlled Gate. Each other Faction receives 1 Elder Sign.", "The Velvet Fan", "Post-Battle", "After any Battle involving the Bloated Woman, choose one Killed or Eliminated enemy Monster or Cultist and place it on this card; that Unit is considered to be out of play. If a Unit is later Recruited or Summoned from this card, its owner pays its Power cost directly to you. Units remain on this card if the Bloated Woman is Killed or Eliminated — they may still be Recruited or Summoned from this card, but no one will be paid Power for this while she is out of play. If she is later returned to play, her controller will receive payments as described above. There is no limit to the number of Units you may have on this card.", "Have all 6 Faction Spellbooks.", "Disaster Looms", "Gather Power Phase", "Your Controlled Gates now earn 1 Elder Sign instead of 2 Power. They still earn 1 Doom during the Doom Phase.", facedown, bloatedWomanHeldHtml(spriteList))
    ...
    // Live-calculated faction-card field (GOO combat/cost, or shared-combat units
    // like Revenants): shows the actual current value when a game is active, "?"
    // otherwise. Owner directive: variable cost/combat GOO/shared units use these,
    // never a bare "?".
    ...
    def bloatedWomanHeldHtml(spriteList : String) : String = {
        ...
        if (imgs.isEmpty) ""
        else s"""<div style="display:flex;flex-wrap:wrap;align-items:flex-end;justify-content:center;gap:0.3vh;">${imgs.mkString("")}</div>"""
    }

    def loyaltyCardIGOO(name : String, cost : String, combat : String, hasSpellbook : Boolean, obtainText : String, ability : String, abilityPhase : String, abilityText : String, spellbookRequirement : String, spellbook : String, spellbookPhase : String, spellbookText : String, facedown : Boolean = false, heldUnits : String = "") = {
        ...
        s"""<table class="loyalty-card-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td></td>
                    <td>
                        ${if (heldUnits.nonEmpty) heldUnits + "<div>&nbsp;</div>" else ""}
                        <div class="black-border">
