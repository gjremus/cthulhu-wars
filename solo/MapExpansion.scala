...
case class BarrierMenuOpenAction(self : Faction, r : Region, f : Faction, effect : |[Spellbook]) extends BaseFactionAction(implicit g => "Battle in " + r + effect./(" with " + _).|(""), implicit g => "n/a — " + f.full) with Soft
...
object LibraryExpansion extends Expansion {
    ...
    def perform(action : Action, soft : VoidGuard)(implicit game : Game) : Continue = action @@ {
        ...
        case SpendOnLibrarianAction(self) =>
            ...
            if ((stayActions ++ moveActions).none)
                EndAction(self)
            else
                Ask(self).each(stayActions ++ moveActions)(identity)
        ...
        // BarrierMenuOpenAction: Soft entry from AttackMainAction's variants when the target
        // holds the barrier. Cancel returns to the target picker via the Soft bridge.
        ...
        // Legacy BarrierCheckAction (preserved for any older paths).
        ...
        case FlipTomeReleaseCultistMainAction(self, tome) =>
            ...
            Ask(self).each(captured)(u =>
                FlipTomeReleaseCultistAction(self, tome, u.ref)
                    .as(u.ref.full, "of", u.faction)("Release a Captured Cultist")).cancel
    ...
    def checkTomeAcquisition()(implicit game : Game) {
        $(TomeBarrier, TomeGuardian, TomeLarvae, TomeYr).foreach { tome =>
            if (game.board.regions.has(tome.region)) {
                ...
                gateOwner match {
                    // Initial acquisition: tome unheld AND a faction now controls its gate.
                    ...
                    // Holder no longer controls the gate — tome stays with holder,
                    // but is now Overdue so the Librarian can target it.
