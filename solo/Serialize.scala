...
class Serialize(val game : Game) {
    ...
    def parseRegion(s : String) : |[Region] = (game.board.regions :+ SL.slumber :+ GC.deep :+ TB.mantle).find(_.id == s)
}

object Serialize {
    val factions = $(GC, CC, BG, YS, SL, WW, OW, AN, TS, FB, DS, TB) ++ $(NeutralAbhoth, LibraryFaction)
