        ...
        def evalA(a : Action) : $[Evaluation] = {
            ...
            a match {
                ...
                case ControlGateAction(_, r, u, _) =>
                    ...
                    // Score is rank value MINUS 100×current-rank so once you reach
            ...
            if (game.battle.any) {
                ...
                a match {
                    ...
                    case MoonbeastReturnPlaceAction(_, _, r, _) =>
                        // Moonbeast return placement — prefer own gate
                    ...
                    case ForcedCultistMoveAction(_, u, dest, _, _) =>
                        ...
                        self.at(dest).any |=> 300 -> "forced move: near own units"
                    ...
                    case CthughaCombatChooseGOOAction(_, _, goo, combat) =>
                        // Always pick the HIGHEST-combat enemy GOO (linear in combat
                        // so the comparison is unambiguous across competing options).
                        true |=> (200 + combat * 100) -> ("cthugha: match GOO combat " + combat)
